...
<blockquote><b>{ktrn} Ket : sudah di dalam list hitam</blockquote></b>
"""
        else:
            await add_to_vars(client.me.id, "BL_ID", chat_id)
            txt = f"""
<blockquote><b>{grp} Group : {message.chat.title}</blockquote></b>\n<blockquote><b>{ktrn} Ket : Succes ditambahkan di list hitam</blockquote></b>
"""
...
async def _(client, message):
    ...
    try:
        ...
        if chat_id not in blacklist:
            response = f"""
<blockquote><b>{grp} Group : {message.chat.title}</blockquote></b>
<blockquote><b>{ktrn} Ket : tidak ada di dalam list hitam</b></blockquote>
"""
        else:
            await remove_from_vars(client.me.id, "BL_ID", chat_id)
            response = f"""
<blockquote><b>{grp} Group : {message.chat.title}</blockquote ></b>
<blockquote><b>{ktrn} Ket : Succes dihapus dari list hitam</blockquote></b>
"""
